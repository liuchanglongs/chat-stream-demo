<template>
	<view class="panel">
		<view class="title">Invitation</view>
		<view class="list">
			<view class="item rela" v-for="item,i in list" :key="i">
				...
				<view class="btm flex j-between a-center">
					...
					<view class="answer flex a-center" @tap="answerQ(item)">
						<u-icon2 name="Edit_more"></u-icon2>
						<view class="txt">answer</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getList() {
				this.$api.pageSocialNoticeInvitationHandle({
					"forward": true,
