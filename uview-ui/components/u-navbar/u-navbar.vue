<template>
	<view class="">
		...
		<!-- 解决fixed定位后导航栏塌陷的问题 -->
		<view class="u-navbar-placeholder" v-if="isFixed && !immersive" :style="{ width: '100%', height: (navbarHeight + statusBarHeight) + 'px' }"></view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			// 整个导航栏的样式
			navbarStyle() {
				let style = {paddingTop:this.paddingTop+'rpx'};
		...
		created() {
			this.statusBarHeight = systemInfo.statusBarHeight ?  systemInfo.statusBarHeight:this.barHeight	
		},
		methods: {
			goBack() {
