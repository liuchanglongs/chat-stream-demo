	...
	<view class="u-subsection" :style="[subsectionStyle]">
		<view class="u-item u-line-1" :style="[itemStyle(index)]" @tap="click(index)" :class="[noBorderRight(index), 'u-item-' + index]"
		 v-for="(item, index) in listInfo" :key="index">
			<view :style="[textStyle(index)]" class="u-item-text u-line-1 rela">
				<text>{{ item.name }}</text>
				<u-badge v-if="badge" type="error" :count="item.count" size="mini" :offset="[-20,-10]" bgColor="#457FD4"></u-badge>
			</view>
		</view>
	...
	export default {
		...
		computed: {
			...
			subsectionStyle() {
				...
				console.log(this.height+'pppppppppppp')
				if (this.mode == 'button') {
			...
			itemBarStyle() {
				...
				if (this.mode == 'button') {
					...
					console.log(style.height+'pppppppppppp')
